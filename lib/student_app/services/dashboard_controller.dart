...
class DashboardController extends GetxController {
  ...
  Future<void> loadAllData({bool forceRefresh = false}) async {
    ...
    try {
      final results = await Future.wait([
        ...
        RemarksService.getRemarks(forceRefresh: forceRefresh).catchError((_) => []),
        ExamsService.getExamStats(forceRefresh: forceRefresh).catchError((_) => <String, dynamic>{}),
      ]);
      ...
      announcements.value = [
        {
          "message": "Unit Test-2 will be conducted from 22nd July. Students are advised to prepare accordingly",
          "department": "Academic Office",
          "color": const Color(0xFF3B82F6),
        },
        {
          "message": "Hostel students must return to campus before 8:00 PM during weekdays",
  ...
  void processAttendanceData(
    ...
    for (var m in classMonths) {
      classChartMonths.add(m.monthName.substring(0, 3));
      classChartData.add({
        'present': m.present,
        'absent': m.absent,
        'outings': m.outings,
    ...
    for (var m in hostelMonths) {
      hostelChartMonths.add(m.monthName.substring(0, 3));
